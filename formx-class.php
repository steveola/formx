...
/////End setting identifiers for coulums
	...
	$all_db_field_array[] = $rowxx[$describe_fnamex];
		...
		//	echo $rowxx[$describe_fnamex]. "<br />";
			...
			if(isset($display->columns->$rowxx[$describe_fnamex])){	
			$field_type_obj = $display->columns->$rowxx[$describe_fnamex]->type;
...
$emtpty_field_array_type[] = $rowxx[$describe_fnamex];
	...
	//newfield, type
	if(isset($addtional_field->columns->$rowxx[$describe_fnamex])){	
	
//	echo "<span style='color:red;'> isset here</span>";
	$add_field_type_obj = $addtional_field->columns->$rowxx[$describe_fnamex]->type;
				if( ($add_field_type_obj == "multipleselect" ) || ($add_field_type_obj == "checkbox" ) || ($add_field_type_obj == "radio" ) || ($add_field_type_obj == "select" )){	
					...
					$add_fieldx = $addtional_field->columns->$rowxx[$describe_fnamex]->newfield;
...
if (isset($display->columns->$name->to_other_tr) && $update_set === true)
{
	{//otyher tr values
...
if ($query = $this->multi_query($insert_sql))
			{	
		if ($query2 = $this->multi_query($delete_multirow_sql.$arrayinsert_sql))
		{
		//echo "Data captured";		
...
\$this->while_data[\"\$dfield\"] .=  \$row_begin;
...
$newfieldtype = $addtional_field->columns->$dfield->type;
//alert($dfield);
//$this->while_data["$newfield"] = "";	 
//$this->while_data["$newfield"] .= $row_begin;
...
//$this->while_data["$newfield"] .= $row_end;
//";
	}
}
}
...
//$this->while_data["$dfield"] .= $row_end;	
...
{
	if (array_key_exists($skey,$this->while_data))
  {
  $while_sorted["$skey"] = $this->while_data["$skey"];
  }	
}
$field_sort_flip = array_flip($field_sort);
$while_sort_interset = array_diff_key($this->while_data,$field_sort_flip);
...
//$dfieldx = $dfield;
	...
	/////end before	addtional_field /////
		...
		} else { if(isset($display->columns) && isset($display->columns->$dfield->type))
			{
				$input_type =  $display->columns->$dfield->type;
			}
		}
...
$invalid_error .= "<$server_validate_all_container class=\"$server_validate_all_class\" style=\"$server_validate_all_style\">";
			...
			{
//error custome
$this_value = $reprint->$dfield; //input value
...
$options_array[$option_display] = $option_value;
			...
			if(isset($display->columns->$dfieldx->values_for_select)){
...
/* */	
	//	print_r($options_array);
		////Display selection option
			$optgroup = array();
			if(isset($display->columns->$dfieldx->optgroup)){
				
			$optgroup = $display->columns->$dfieldx->optgroup;
			}
				...
				foreach($optgroup as $each_optgroup => $optgroup_value){
					...
					if(isset($display->columns->$dfieldx->selected))
					{
						if(in_array($optiongroup_value, $display->columns->$dfieldx->selected))
						{$selected = "selected=\"selected\"";}	
					}
				...
				foreach ($options_array as $option => $value ) { 
					...
					if(isset($display->columns->$dfieldx->selected))
					{
						if(in_array($value, $display->columns->$dfieldx->selected))
						{$selected = "selected=\"selected\"";}	
					}
...
//}
	...
	{  
		...
		$element_separator = "";
		if(isset($display->columns->$dfieldx->element_separator)){
		$element_separator = $display->columns->$dfieldx->element_separator;
		}
...
///////db options
	...
	if(isset($display->columns->$dfieldx->values_for_select)){		
		 $options_array =	$display->columns->$dfieldx->values_for_select;
	}
		}	 
		foreach ($options_array as $option_display => $option_value ) 
			...
			if(isset($display->columns->$dfieldx->checked))
			{
				if(in_array($option_value, $display->columns->$dfieldx->checked))
				{
						$selected = "checked=\"checked\"";
				}
...
foreach($array_raw_select as $skey => $sdata)
{
	...
	////End setting for other table	
							...
							if(isset($display->columns->$dfieldx->checked))
			{
				if(in_array($value, $display->columns->$dfieldx->checked))
				{
						$selected = "checked=\"checked\"";
				}
...
//FORM ELEMENT WRITER
	$this->while_data["$dfield"] .= $this->separator['row_begin'];
	
	$this->while_data["$dfield"] .= $column_textdisplay_open;
	
	$this->while_data["$dfield"] .= $before_label;

	$this->while_data["$dfield"] .= $this->input_label["$dfield"];//LABEL

	$this->while_data["$dfield"] .= $after_label;
	$this->while_data["$dfield"] .= $column_textdisplay_close;

	
	$this->while_data["$dfield"] .= $column_formdisplay_open;
	$this->while_data["$dfield"] .= $before_element;
	$this->while_data["$dfield"] .= $error_before_element;
	
	
	$this->while_data["$dfield"] .= $this->input_element["$dfield"]; ///INPUTS
		
		
		
	$this->while_data["$dfield"] .= $error_after_element;
	$this->while_data["$dfield"] .= $after_element;	
	
	$this->while_data["$dfield"] .= $column_formdisplay_close;
	
	$this->while_data["$dfield"] .= $this->separator['row_end'];	
...
$add_free_field  = array( //must add proccessors   ////when $add_free_field is declared, parameter 2 must be same as that set in $display->columns->type
					...
					array("free_no_display","textarea"),
					//array("","");
						);
...
array(	'foo' => 'bar',
		...
		'update_failure' => "\$this->alert(\"Update failed\");",
		...
		'faculty_code' => (object) array ( 	'type'=> 'checkbox', //checkbox, multipleselect							
													...
													'element_separator' => '<br />', ///HTML TAG
													'extra_data' => "\$time = time(); \$xxtime = time(); \$extra=404;",    ////e.g 'extra_data' => "\$time = time();",
												///	'checked' => array('2','3')
												),
